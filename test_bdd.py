...
import unittest
import sqlite3
from fichiersTemporaires import writeFichierTemporaire, getFichierTemporaire, readFichierTemporaire, readLineFichierTemporaire
...
#         self.assertEqual(True, bool2FichiersOntMemeContenu)
    ...
    def test_SYNTAXE_re_nommee(self):
        ...
        ligne = "1,2,3,12,22,30 1 2016 P1"
        #item 148
        pat = re.compile(r"""(?P<jours>^(\d+,*)+)\s #jours
        (?P<mois>\d{1,2})\s     #mois
        (?P<annee>\d{4,4})\s    #annee
        (?P<poste>\w{2,2})     #poste
        """,
        re.VERBOSE)
